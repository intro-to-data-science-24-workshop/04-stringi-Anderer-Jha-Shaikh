...
**1. Working with temporal data from different locales**: use `stri_datetime_parse`to homogenize temporal data. 
--
<br>
...
**2. General Transforms**: use `stri_trans_general` to transliterate characters from one script to another script of your choosing. 
--
<br>
...
**3. Text Boundary Analysis**: To split text into lines/words/characters, count characters or locate particular text units (e.g. the 4th word of a sentence), use commands like `stri_count_boundares`, `stri_split_boundaries` or `stri_locate_boundaries`.
--
<br>
...
**4. Extracting & Replacing Substrings**: use `stri_sub`to extract and replace substrings.
--
<br>
...
**5. Concatenating Strings**: to run concatenation-based operations, use `stri_join`, `stri_dup`, %s+%, or `stri_flatten`.
--
<br>
...
**6. Trimming**: to remove uneccessary white spaces, use `stri_trim`.
--
<br>
